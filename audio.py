...
def _griffin_lim_tensorflow(S, hparams):
	...
	with tf.variable_scope('griffinlim'):
		...
		y = tf.contrib.signal.inverse_stft(S_complex, hparams.win_length, hparams.hop_length, hparams.n_fft)
		for i in range(hparams.griffin_lim_iters):
			est = tf.contrib.signal.stft(y, hparams.win_length, hparams.hop_length, hparams.n_fft)
			angles = est / tf.cast(tf.maximum(1e-8, tf.abs(est)), tf.complex64)
			y = tf.contrib.signal.inverse_stft(S_complex * angles, hparams.win_length, hparams.hop_length, hparams.n_fft)
